...
class HumanAgent(AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        if (self._basic_agent is None):
            if (self._ego_vehicle is None):
                print("ego vehicle is not available (NoneType)")
            print("set basic agent")
            self._basic_agent = BasicAgent(self._ego_vehicle, 20);
            self._basic_agent.set_global_plan(self._global_plan_world_coord)
        self.agent_engaged = True
        time.sleep(0.1)
        return self._basic_agent.run_step() #self.current_control
    ...
    def set_global_plan(self, global_plan_gps, global_plan_world_coord):
        ...
        """
        super(HumanAgent, self).set_global_plan(global_plan_gps,
